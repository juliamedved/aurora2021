...
export default class Walking extends Steering {
	constructor(owner, target, force = 1, ownerSpeed = 40, walkingRadius = 70) {
	...
	findTarget() {
		...
		const randwalkingRadius = Phaser.Math.RND.between(1, this.walkingRadius);
		const randAngle = this.degToRad(Phaser.Math.RND.between(0, 360));
		this.target = new Vector2(this.owner.x + randwalkingRadius * Math.sin(randAngle), this.owner.y + randwalkingRadius * Math.cos(randAngle));
